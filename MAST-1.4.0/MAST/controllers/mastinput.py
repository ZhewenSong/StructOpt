...
class MASTInput(MASTObj):
    ...
    def check_independent_loops(self):
        ...
        self.logger.info("%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
        work_dirs = [] ## saving all the working directory names from loop
        try:
            ...
            if len(loopfiles) == 0:
                self.set_up_recipe()
                work_dirs.append(os.path.basename(self.working_directory))
            else:
    ...
    def set_sysname(self):
        ...
        ing = self.input_options.get_item("ingredients") 
        ingkey = ing.keys()
        for k in ingkey:
            if 'mast_program' in ing[k].keys() and ing[k]['mast_program'] == 'structopt':
                frags = ing[k]['atomlist'].split("'") 
                element_str = ""
                for n in range(len(frags)/2):
                    element_str += frags[2*n+1]
