...
#             os.chdir(mycwd)
    ...
    def set_up_program_input_whole_stem(self):
        ...
        if 'potential_file' in self.structopt_parameters:
            ...
            if not Optinit.pot_file:
                Optinit.pot_file = os.path.basename(self.structopt_parameters['potential_file'])
        if 'stem_ref' in self.structopt_parameters:
            newlocation = os.path.join(ingpath,os.path.basename(self.structopt_parameters['stem_ref']))
            shutil.copyfile(self.structopt_parameters['stem_ref'],newlocation)
        if 'psf_file' in self.structopt_parameters:
            newlocation = os.path.join(ingpath,os.path.basename(self.structopt_parameters['psf_file']))
            shutil.copyfile(self.structopt_parameters['psf_file'],newlocation)
        #if 'stem_script' in self.structopt_parameters:
        #    newlocation = os.path.join(ingpath,os.path.basename(self.structopt_parameters['stem_script']))
        #    shutil.copyfile(self.structopt_parameters['stem_script'],newlocation)
        if Optinit.restart_optimizer:
            Optinit.population = population_list
            Optinit.BESTS = bests_list
            Optinit.write(inputfile)
            ...
            Optinit.write(inputfile,restart=False)
        self.keywords['program_keys']['mast_exec'] = "/share/apps/mvapich2/tam_mvapich2-1.9a2/usr/local/bin/mpirun /share/apps/EPD_64bit/epd_free-7.3-2-rh5-x86_64/bin/python %s structoptinput.txt > output.txt"%os.path.join(os.environ['MAST_CONTROL'],'Optimizer.py')
        return
    ...
    def evaluate_structures_for_fitness(self, MyOpti):
        ...
        if MyOpti.convergence:
            ...
            self.clear_folders()
            os.system('rm '+os.path.join(self.keywords['name'],MyOpti.filename+'-rank0','POSCAR*'))
            rankpath = os.path.join(self.keywords['name'],MyOpti.filename+'-rank0')
            os.mkdir(os.path.join(rankpath,'VASPFiles'))
            os.system("mv %s %s/."%(os.path.join(self.keywords['name'],'20??????_??????'),os.path.join(rankpath,'VASPFiles')))
            return MyOpti
        else:
