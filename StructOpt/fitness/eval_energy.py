...
def eval_energy(Optimizer, individ):
    ...
    if 'MAST' in Optimizer.calc_method:
        ...
        logger.info('Received individual index = {0} from MAST. Returning with no evaluation'.format(individ.index))
    else:
        if Optimizer.parallel: 
            rank = MPI.COMM_WORLD.Get_rank()
        else:
            rank = 0
        logger.info('Received individual HI = {0} for energy evaluation'.format(
        ...
        if Optimizer.parallel:
            calc = setup_energy_calculator(Optimizer)
            if Optimizer.fixed_region:
                ...
                Optimizer.static_calc = setup_energy_calculator(Optimizer)
...
def check_min_dist(Optimizer, totalsol, type='Defect', nat=None, min_len=0.7, STR=''):
    if type=='Defect' or type=='Crystal' or type=='Surface':
        ...
        if not Optimizer.calc_method=='LAMMPS':
